...
namespace GeometryAPI
{
    ...
    public class Circle : Figure, IAreaMeasurable, IFlat, ITransformable
    {
        ...
        new void Scale(Vector3D scaleCenter, double scaleFactor)
        {
    ...
    {
        private Vector3D CenterBottom
        {
            get { return this.vertices[0]; }
            set { this.vertices[0] = value; }
        }
        private Vector3D CenterTop
        {
            get { return this.vertices[1]; }
            set { this.vertices[1] = value; }
        }
        ...
        public double GetArea()
        {
            double height = Math.Sqrt((CenterTop.X - CenterBottom.X) * (CenterTop.X - CenterBottom.X) + (CenterTop.Y - CenterBottom.Y) * (CenterTop.Y - CenterBottom.Y) + (CenterTop.Z - CenterBottom.Z) * (CenterTop.Z - CenterBottom.Z));
            return this.Radius * 2 * Math.PI * height +
    ...
    // TODO: HERE add my figure controller
    public class myFigureController : FigureController
    {
        public override void ExecuteFigureCreationCommand(string[] splitFigString)
        {
            switch (splitFigString[0])
            {
                case "circle":
                    ...
                    break;
                case "cylinder":
                    Vector3D centerBottom = Vector3D.Parse(splitFigString[1]);
                    Vector3D centerTop = Vector3D.Parse(splitFigString[2]);
                    double radiusTopBottom = double.Parse(splitFigString[3]);
                    currentFigure = new Cylinder(centerBottom, centerTop, radiusTopBottom);
                    break;
            }
        ...
        protected override void ExecuteFigureInstanceCommand(string[] splitCommand)
        {
            ...
            {
                case "area":
                    if ((currentFigure as IAreaMeasurable) != null)
                    {
                        Console.WriteLine("{0:0.00}", ((IAreaMeasurable)currentFigure).GetArea());
                    }
                    else
                    {
                    ...
                    break;
                case "volume":
                    if ((currentFigure as IVolumeMeasurable) != null)
                    {
                        Console.WriteLine("{0:0.00}", ((IVolumeMeasurable)currentFigure).GetVolume());
                    }
                    else
                    {
                        Console.WriteLine("undefined");
                    ...
                    break;
                case "normal":
                    if ((currentFigure as IFlat) != null)
                    {
                        Console.WriteLine("{0:0.00}", ((IFlat)currentFigure).GetNormal());
                    }
                    else
                    {
                        Console.WriteLine("undefined");
